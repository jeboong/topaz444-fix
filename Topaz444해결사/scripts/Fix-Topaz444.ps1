...
#  - 업데이트로 사라진 ProRes 4444(444) 인코더 옵션 복구
...
Write-Host "   Topaz Video 444 + 화면꺼짐 원클릭 해결사" -ForegroundColor Cyan
Write-Host "=====================================================" -ForegroundColor Cyan
Write-Host ""
...
$entryJson = @'
{
  ...
  "profile": "4444",
  "allowsAlpha": 1,
  "ffmpegOpts": "-c:v prores_ks -profile:v 4 -vendor apl0 -tag:v ap4h -pix_fmt yuv444p10le -metadata:s:v:0 encoder=AppleProRes4444 -f mov -brand \"qt  \" -movflags use_metadata_tags+write_colr",
  "ext": [ "mov" ],
  "os": "windows|linux",
  "minSize": [ 1, 1 ],
  "maxSize": [ 16386, 16386 ],
...
$encoders   = @(Get-Content -LiteralPath $encoderPath -Raw -Encoding UTF8 | ConvertFrom-Json)
$entryFlat  = $entry | ConvertTo-Json -Depth 20 -Compress
$existing   = $encoders | Where-Object { $_.id -eq $entry.id }

if ($existing -and (($existing | ConvertTo-Json -Depth 20 -Compress) -eq $entryFlat)) {
  Skip "444 인코더가 이미 최신 상태입니다."
} elseif ($WhatIfOnly) {
  Log ("  -> 444 인코더가 " + $(if ($existing) { "구버전입니다. 갱신 대상." } else { "없습니다. 추가 대상." })) 'Yellow'
} else {
  ...
  $newJson = ConvertTo-Json -InputObject @($updated) -Depth 20
  [IO.File]::WriteAllText($encoderPath, $newJson, (New-Object Text.UTF8Encoding($false)))

  # 저장 후 JSON 이 정상인지 검증
  $null = Get-Content -LiteralPath $encoderPath -Raw | ConvertFrom-Json
  Done "video-encoders.json 에 ProRes 4444 (444) 항목 추가 완료"
  Log  "  ※ 참고: 토파즈가 또 업데이트되면 이 파일이 초기화될 수 있습니다. 그때 이 해결사를 다시 실행하면 됩니다." 'DarkCyan'
}
...
# 3-2. Windows TDR(그래픽 드라이버 응답 대기시간) 연장: 2초 -> 60초
...
Write-Host "   2. Topaz Video 를 켜고 출력 코덱에 ProRes 4444 가" -ForegroundColor White
Write-Host "      다시 보이는지 확인해주세요." -ForegroundColor White
Write-Host ""
